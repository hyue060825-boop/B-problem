...
[单独查看右图（说明已移到图下）](coverage_cells.png) · [右图PDF](coverage_cells.pdf) · [右图SVG](coverage_cells.svg)

可以把蓝圆看成一张“源可能藏在哪里”的地图。为了验证31个固定站点有没有漏掉某个位置，程序把地图切成一块块小方格，逐块做数学检查。

对任意一个方格，程序从31个站点中挑出一组附近站点，同时证明两件事：第一，源无论落在方格里的哪个位置，到这些站点的距离都不超过999米；第二，这些站点从几何上包围了整个方格，因此源无论朝哪个方向发射，都不可能让这些站点全部落在背面。源和站点恰好重合的特殊情况由其他邻站另行补证。

较大的方格如果不能直接通过这组充分条件，就继续切成四个更小的方格再检查，所以图中格子大小不同。颜色仅表示切分深度：深色更细，浅色更粗。深色不是信号强，也不是探测概率高，更不是那里实际有源；它只反映证明算法在那个区域用了更细的划分。

“2152个见证单元”意味着2152块区域均有相应的数学依据；“未决单元为0”意味着没有留下无法证明的区域，并非测得0个信号。全部区域合起来覆盖整个可能源域，说明不存在能躲过所有31个站点的源位置和朝向。这是连续区域的验证，不是只试2152个点。

整个推理是：**如果存在源，至少一个站点一定能测到；而同一频道的31个站点都有效测得无信号，所以在既定接收规则下该频道没有源。** 判断依赖前述半径、定向半平面和无额外漏检等条件；只看图片本身不能代替证书验证。
