...
def provenance():
    paths=list(Path('solution').rglob('*.py'))+list(Path('bsim').glob('*.py'))+list(Path('scripts').glob('*.py'))
...
def verify_checkpoint_code(data):
    ...
    mismatch=[p for p in critical if recorded.get(p)!=hashlib.sha256((root/p).read_bytes()).hexdigest()]
