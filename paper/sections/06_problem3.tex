...
\subsection{正式测试结果}
采用最终权重进行三次正式运行，并按题目规定的四列格式整理结果，见\cref{tab:q3-formal}。成功清除数按不同频道的成功清除响应计数；平均定位清除时间按退出时累计虚拟时间除以成功清除数计算。三次总虚拟时间分别为 $3456.158304$、$2949.766267$ 和 $3731.298109\,\mathrm{s}$。
...
\hline
测试 1（编码待补） & 13 & 265.86 & 1.389 \\
\hline
测试 2（编码待补） & 11 & 268.16 & 1.274 \\
\hline
测试 3（编码待补） & 13 & 287.02 & 1.136 \\
\hline
\end{tabularx}
\end{table}

程序运行时间按模拟器返回的 \texttt{/exit} 与 \texttt{/enter} 现实时间戳之差计算，计入进入后的在线决策与通信，不计进入前的模型载入。表中虚拟时间与现实时间属于不同计时口径。

原始客户端记录为 \texttt{q3\_1.jsonl}、\texttt{q3\_2.jsonl} 和 \texttt{q3\_3.jsonl}，权重摘要均与本文最终 Q3 模型一致。六份客户端记录均未包含正式案例编码及实际源总数，故不以机器人编号替代案例编码，也不据此另报源级清除比例；案例编码须从模拟器成绩页补齐。题目要求的模拟器导出日志仍需以原文件名另行归档。
