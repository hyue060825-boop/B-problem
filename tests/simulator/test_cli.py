...
from tests.simulator.support import ROOT
...
class CLI(unittest.TestCase):
    ...
    def test_profile_block_and_replay_cli(self):
        ...
        result=self.runcli('replay','--fixture','tests/fixtures/simulator/timing.json','--trace','tests/fixtures/simulator/timing.trace.json')
    ...
    def test_source_audit_uses_project_materials_from_another_directory(self):
        import tempfile
        with tempfile.TemporaryDirectory() as directory:
            result = subprocess.run([sys.executable, '-m', 'bsim', 'audit-rules'],
    ...
    def test_separate_server_and_socket_lifecycle(self):
        ...
        process=subprocess.Popen([sys.executable,'-m','bsim','serve','--profile','fixture_conformance',
                                  '--fixture','tests/fixtures/simulator/timing.json','--port',str(port)],cwd=ROOT,
