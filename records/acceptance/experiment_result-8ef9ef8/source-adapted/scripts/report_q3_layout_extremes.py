...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input',default='runs/q3_layout_extremes_20260913_v2');a=ap.parse_args()
    ...
    lines=['# Q3：10–15 源的有利/不利分布搜索','',
           f"使用原最佳基线 `runs/q3_legacy_deadline_20260913/best.pt`。共构造并测评 {summary['total_evaluated']} 个样例，每个源数 {summary['counts_per_N']['10']} 个；失败数 {summary['failures']}。下表为本轮找到的最短/最长总虚拟耗时，不是全局极值的数学证明，也不是随机测试的均值或置信区间。",'',
