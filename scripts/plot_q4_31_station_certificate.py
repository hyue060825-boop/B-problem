...
def main():
    ...
    fig,axes=plt.subplots(1,3,figsize=(19,7.6),gridspec_kw={'width_ratios':[1.15,1,1]})
    fig.subplots_adjust(left=.035,right=.985,top=.80,bottom=.26,wspace=.25)
    ...
    ax.text(.5,.97,'2152 个见证单元；未决单元 = 0',transform=ax.transAxes,ha='center',fontsize=9,color='#0f172a')
    ax.text(.5,.03,'深浅仅表示四叉树细分层级，不是采样成功率',transform=ax.transAxes,ha='center',fontsize=8,color='#475569')
    for ax in axes:
        ax.set_title(ax.get_title(),pad=28)
        ax.set_aspect('equal');ax.grid(alpha=.12)
        for spine in ax.spines.values():spine.set_color('#cbd5e1')
    fig.text(.045,.185,'布局保证\n黑点为31站；蓝圆为半径1800 m的可能源域。\n任意源落在闭三角形内，到三个顶点均小于1000 m。\n包含圆外站点；编号不表示访问顺序。',fontsize=10,va='top',linespacing=1.6)
    fig.text(.377,.185,'方向保证\n'+r'$p=\sum_i\lambda_i v_i\ \Rightarrow\ \sum_i\lambda_i\,u^T(v_i-p)=0$'+'\n因此不可能所有顶点均在背向开半平面。\n源与站点重合时，由周围非重合站点补证。',fontsize=10,va='top',linespacing=1.6)
    fig.text(.705,.185,'精确证据\n有理数验证通过：2152 个见证单元 + 72 个域外单元；\n13 处重合点保护；认证距离 ≤ 999 m。\n原证书坐标与当前策略发送的站点坐标精确一致。',fontsize=10,va='top',linespacing=1.6)
