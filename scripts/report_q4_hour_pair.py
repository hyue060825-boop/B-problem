...
RUN=ROOT/'runs/q4_baseline_8gpu_1h_20260913'
...
def main():
    ...
    original=ROOT/'runs/q4_legacy_deadline_20260913/best.pt'
    ...
    for label,path in [('baseline',data['baseline_checkpoint']),('candidate',data['checkpoint'])]:
        ...
        assert all(sha(RUN/'source'/p)==h for p,h in critical.items())
