...
def main():
    ...
    source=ROOT/'results/final-20260913/q34_deadline_test3000_20260913/q4_best_samples.json';original=json.loads(source.read_text());checkpoint=ROOT/'results/final-20260913/q4_legacy_deadline_20260913/best.pt'
    log,ev,official=log_events(ROOT/'results/official/q4-20260913/logQ4.jsonl')
    ...
    (out/'manifest.json').write_text(json.dumps(dict(checkpoint=str(checkpoint),sha256=log[0]['checkpoint_sha256'],original_samples=str(source),samples_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),official_log_sha256=hashlib.sha256(Path(ROOT/'results/official/q4-20260913/logQ4.jsonl').read_bytes()).hexdigest(),note='original aggregate data lack primitive timestamps; same scenarios replayed with instrumentation; live wall feature retained'),indent=2))
