"""Construct and search Q4 layout extrema, holding non-layout inputs fixed."""
import argparse
...
MODES=('near_start','compact','area','edge_outward','edge_inward','edge_tangent','two_clusters','thin_line')
CHECKPOINT=ROOT/'runs/q4_legacy_deadline_20260913/best.pt'
...
def make_case(n,mode,seed):
    ...
    xy=project(xy);outward=np.degrees(np.arctan2(xy[:,1],xy[:,0]))%360
    if mode in ('near_start','edge_inward'):headings=(outward+180)%360
    elif mode in ('edge_outward','two_clusters'):headings=outward
    elif mode=='edge_tangent':headings=(outward+rng.choice([-90,90],n)+rng.uniform(-3,3,n))%360
    else:headings=rng.uniform(0,360,n)
    sources=[Source(i+1,float(x),float(y),1000.,'directional' if i<n//2 else 'omni',float(headings[i]) if i<n//2 else None) for i,(x,y) in enumerate(xy)]
    return fixture(Scenario(f'LOCAL-Q4-LAYOUT-{seed}',4,'official_constraints',tuple(sources)),seed,mode)
...
def mutate(case,seed,round_index):
    ...
    n=len(sources);xy=np.array([[s['x'],s['y']] for s in sources]);headings=np.array([s['heading'] or 0 for s in sources])
    kind=seed%4
    if kind==0:
        ...
        xy=xy@rot;headings+=np.degrees(angle)
    else:
        chosen=rng.choice(n//2,size=max(1,n//4),replace=False)
        headings[chosen]+=rng.normal(0,30/(round_index+1),len(chosen))
    xy=project(xy)
    for i,s in enumerate(sources):
        s['x'],s['y']=map(float,xy[i])
        if s['kind']=='directional':s['heading']=float(headings[i]%360)
    return fixture(Scenario(f'LOCAL-Q4-LAYOUT-{seed}',4,'official_constraints',tuple(Source(**s) for s in sources)),seed,'mutated:'+case['mode'].split(':')[-1])


def evaluate(job):
    case,record,clock_mode=job
    spec=case['scenario'];scenario=Scenario(spec['label'],4,spec['scope'],tuple(Source(**s) for s in spec['sources']))
    ...
    try:env=TrainingEnv(4,case['seed'],max_macros=400)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',default='runs/q4_layout_extremes_20260913')
    ...
    manifest=dict(checkpoint=str(CHECKPOINT),checkpoint_sha256=hashlib.sha256(CHECKPOINT.read_bytes()).hexdigest(),
                  ...
                  fixed_controls=dict(channels='1..N',directional_count='floor(N/2)',radius_m=1000,error_deg=0),
    ...
    seed=2010000000;best={};worst={};failures=[];rows=[];case_by_seed={};start=time.perf_counter()
    initial=[]
