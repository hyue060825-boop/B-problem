...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--input',default='runs/q4_layout_extremes_20260913');a=ap.parse_args()
    ...
    lines=['# Q4：10–15 源的有利/不利分布搜索','',
           f"使用原最佳基线 `runs/q4_legacy_deadline_20260913/best.pt`。共构造并测评 {summary['total_evaluated']} 个样例，每个源数 {summary['counts_per_N']['10']} 个；失败数 {summary['failures']}。下表为本轮找到的最短/最长总虚拟耗时，不是全局极值的数学证明，也不是随机测试的均值或置信区间。",'',
    ...
    lines+=['','- extremes.csv：12 个代表样例的汇总。',
            ...
            '复跑单个样例（输出目录须不存在）：','',
            '```bash','./scripts/run_python.sh scripts/search_q4_layout_extremes.py --replay runs/q4_layout_extremes_20260913/N10/worst_fixture.json --output runs/q4_layout_extremes_20260913/N10_worst_replay','```']
    ...
    for i,n in enumerate(ns):
        for j,label in enumerate(('best','worst')):
            ...
            case=json.loads(Path(r['fixture']).read_text());trace=json.loads((root/f'N{n}/{label}_trace.json').read_text())
            ...
            ax.grid(alpha=.15)
    fig.suptitle('橙色为定向源（箭头示朝向），蓝色为全向源；灰线为实际路线，虚线为源位置边界',fontsize=12)
