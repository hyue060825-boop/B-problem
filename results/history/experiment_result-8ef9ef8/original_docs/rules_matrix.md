...
证据版本见 `rules/source_manifest.json`；三份官方原件与任务书参考哈希一致。此表的PASS仅表示对应公开规则在列出的测试范围通过，不表示隐藏生成过程一致。`EXPLICIT`为官方明示；`DERIVED`为从明示规则推导；`UNSPECIFIED`为缺充分定义；`CONFLICT`用于证据冲突。本轮未确认必须改变实现的物理/协议冲突，日志表述歧义单独记录。
...
| R28 | 无BOM UTF-8对象，无重复键，≤65536字节、≤16层 | 附件2 §5.1 | EXPLICIT | protocol.validate/depth/pairs | Protocol.test_json_invalids / test_headers_paths_identifiers_size_depth | 深度计数约定G07 |
