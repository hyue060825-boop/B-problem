...
def main():
    ...
    log,ev,official=log_events('/data5/hy/logQ4.jsonl')
    ...
    (out/'manifest.json').write_text(json.dumps(dict(checkpoint=str(checkpoint),sha256=log[0]['checkpoint_sha256'],original_samples=str(source),samples_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),official_log_sha256=hashlib.sha256(Path('/data5/hy/logQ4.jsonl').read_bytes()).hexdigest(),note='original aggregate data lack primitive timestamps; same scenarios replayed with instrumentation; live wall feature retained'),indent=2))
