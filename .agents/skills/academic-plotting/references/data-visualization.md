...
Pattern library for quantitative figures. Examples are starting points, not
automatic analysis decisions. Preserve source data and record filtering,
aggregation, normalization, fitting, and scale choices. Use log scales only
for valid domains and meaningful interpretations. Shared axes require comparable
units and scales. Highlight only a method identified by the user or context,
without hiding baseline differences. Follow the main skill's QA policy.

## Recipe Dependencies

Use only the imports and capabilities needed for the selected pattern. Seaborn,
scipy, SciencePlots, and a LaTeX installation are optional; their absence does
not block other matplotlib patterns. Use existing tools or a suitable available
renderer, and do not install packages solely to follow an example. The main
Skill defines capability fallback and authorization boundaries.
...
### "Okabe-Ito" (Accessibility-Oriented Palette Example)
...
The following are optional style choices. Preserve scientific meaning and
distinguishable baselines; apply highlights only to a method identified in the
FigureSpec. Use supported uncertainty and final-size QA.
...
| Uncertainty needs to be shown | Plot an interval supported by the data and label its statistical meaning |
| Generic blue bars | "Ocean Dusk" palette + white edge between bars |
| Baselines need comparison | Use distinguishable labels/markers/colors; an identified focal method may receive an accent |
...
These are historical starting dimensions. Follow [style-guide.md](style-guide.md)
and the selected venue/year/template; validate at the final display size.
...
def plot_training_curves(data, metric="Loss", save_path="figures/fig_training.pdf",
    ...
    preview_path: optional explicit output path distinct from save_path
    """
    from pathlib import Path
    if preview_path is not None and Path(preview_path).resolve() == Path(save_path).resolve():
        raise ValueError("Preview and primary output paths must differ")
    ...
    # Keep the default linear scale unless the FigureSpec calls for a log scale
    # and the supplied values have a valid positive domain.
...
This example shows mean ± standard deviation, not a confidence interval. Use only
uncertainty estimates supported by supplied runs or an explicitly defined
statistical procedure. State the quantity and method; do not invent bands for a
single observation or add significance markers for style.
...
def plot_leaderboard(models, scores, highlight_idx=-1, xlabel="Score",
    ...
    if highlight_idx >= 0:
        colors[highlight_idx] = OUR_COLOR
...
Use when the FigureSpec calls for log-log axes and the data have a valid positive
domain. Fitting is optional and must be justified by the analysis specification;
record its assumptions and method. Invalid values are not silently filtered.
...
def plot_scaling(sizes, metrics, fit_line=False, xlabel="Parameters",
    ...
    if (sizes.ndim != 1 or metrics.ndim != 1 or sizes.size == 0
            or sizes.size != metrics.size
            or not np.all(np.isfinite(sizes)) or not np.all(np.isfinite(metrics))
            or np.any(sizes <= 0) or np.any(metrics <= 0)):
        raise ValueError("Log-log plotting requires paired finite positive values")
    if fit_line and np.unique(sizes).size < 2:
        raise ValueError("A fitted line requires at least two distinct sizes")
    fig, ax = plt.subplots(figsize=FIG_ICML_SINGLE)
...
Recipe filenames are examples. Adapt exports to the required format; auxiliary
PNG previews are optional. Functions that demonstrate both outputs do not impose
a dual-export completion requirement.

### Export Required Formats

Follow [Figure Output Policy](../SKILL.md#figure-output-policy). The paired exports
below illustrate a publication artifact plus optional preview, not a mandatory
dual-format deliverable. Check representation and resolution at final size.

```python
# PDF for LaTeX; verify whether its contents are vector or mixed
fig.savefig("figures/fig_name.pdf", bbox_inches="tight", pad_inches=0.05)

# Optional raster preview for README/slides or a requested raster deliverable
...
Every quantitative figure should retain reproducible plotting code and data provenance.
Adapt this skeleton to the actual data and requested outputs; the PNG export is optional:
