...
from .paths import ROOT, FIXTURES, PROBLEM
...
def main(argv=None):
    ...
    a=sub.add_parser('audit-rules');a.add_argument('--sources',type=Path,default=PROBLEM,help='包含 SHA256SUMS 的题面目录');a.add_argument('--output')
    ...
    a=sub.add_parser('compare-kernels');a.add_argument('--profile',default='fixture_conformance');a.add_argument('--fixture',type=Path,default=FIXTURES/'timing.json');a.add_argument('--steps',type=int,default=5000);a.add_argument('--output')
    a=sub.add_parser('benchmark');a.add_argument('--device',default='cpu');a.add_argument('--fixture',type=Path,default=FIXTURES/'timing.json');a.add_argument('--num-envs',type=int,default=64);a.add_argument('--steps',type=int,default=128);a.add_argument('--output')
    ...
    try:
        ...
        if args.command=='audit-rules':
            ...
            report=audit_sources(args.sources);emit(report,args.output)
            return int(report['status']!='PASS')
        if args.command=='validate-fixtures':
            sys.path.insert(0,str(ROOT))
            suite=unittest.defaultTestLoader.discover(str(ROOT/'tests/simulator'),top_level_dir=str(ROOT))
